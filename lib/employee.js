...
module.exports = {
    ...
    getDeptBudget: function (callback) {  
        const sql = 'SELECT d.name as Department, FORMAT(SUM(r.salary),0) AS Total\
        FROM employee as e INNER JOIN role as r on e.role_id = r.id \
        INNER JOIN department as d ON r.department_id = d.id \
       LEFT JOIN employee as m on e.manager_id = m.id GROUP BY Department';
           connection.query(sql, function(err, data) {
            if (err) throw err;
            if (data.length > 0) {
             dataArr = JSON.parse(JSON.stringify(data));
    ...
    inqAddEmployee: function (defaults, roles, managers, callback) { 
        ...
        selEmployee: function (employees, callback) {
            console.log("select emp",employees)
            inquirer.prompt({
                type: 'checkbox',
                name: 'empSel', 
